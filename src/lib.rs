...
#[pg_extern]
fn pg_modify_stats(json_dump: String, statistic_type: String, statistic_value: String) -> String {
    // Parse json
...
fn get_table_rows() -> Vec<i32> {
    Spi::run("CREATE TABLE demo_table (value INTEGER);").unwrap();
    Spi::run("INSERT INTO demo_table (value) VALUES (12), (123), (142);").unwrap();
    ...
    let result = Spi::connect(|client| {
        ...
        while let Some(row) = tup_table.next() {
            let num: Result<Option<i32>, spi::SpiError> = row["value"].value::<i32>();
            println!("num: {:?}", num);
            results.push(num);
        }
