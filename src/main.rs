...
async fn table_dump(mut req: Request<PgPool>) -> tide::Result {
    ...
    let query = format!("SELECT oid, relnatts from pg_class where relname='{}'", relname);
    let ClassInfo { oid, relnatts } = sqlx::query_as(&query)
        .fetch_one(pool)
        .await?;
    println!("oid: {} num columns: {}\n", oid, relnatts);
    let class_query = format!("SELECT pg_class_dump({})", oid);
    let class_dump: (String, ) = sqlx::query_as(&class_query)
        .fetch_one(pool)
        .await?;
    ...
    for i in 1..relnatts + 1 {
        let stat_query = format!("SELECT pg_statistic_dump({}, CAST ({} as SMALLINT))", oid, i);
        let stat_dump: (String, ) = sqlx::query_as(&stat_query)
            .fetch_one(pool)
            .await?;
        println!("Received dump for starelid {}, staattnum {}\n{}\n", oid, i, stat_dump.0);
    ...
    for i in 1..relnatts + 1 {
        ...
        let att_dump: (String, ) = sqlx::query_as(&att_query)
            .fetch_one(pool)
            .await?;
        println!("Received dump for oid {}, attcol {}\n{}\n", oid, i, att_dump.0);
        att_dumps.push(att_dump.0);
    }

    let full_dump = TableDump { class_info: class_dump.0, stats_info: stat_dumps, atts_info: att_dumps };
    let json_str = serde_json::to_string(&full_dump).unwrap();
    Ok(format!("{json_str}\n").into())
}

async fn table_load(mut req: Request<PgPool>) -> tide::Result {
    let TableDump { class_info, stats_info, atts_info } = req.body_json().await?;
    let pool = req.state();

    let class_load_query = format!("SELECT pg_class_load('{}')", class_info);
    let class_loaded: (bool, ) = sqlx::query_as(&class_load_query)
        .fetch_one(pool)
        .await?;
    if !class_loaded.0 {
    ...
    for stat in stats_info.iter() {
        ...
        let stat_loaded: (bool, ) = sqlx::query_as(&stat_load_query)
            .fetch_one(pool)
            .await?;
        if !stat_loaded.0 {
    ...
    for att in atts_info.iter() {
        ...
        let att_loaded: (bool, ) = sqlx::query_as(&att_load_query)
            .fetch_one(pool)
            .await?;
        if !att_loaded.0 {
